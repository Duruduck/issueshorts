...
function buildImagePrompts(topic, count, opts) {
  ...
  const colorMap = {"밝고 경쾌한":"bright vibrant warm","다크·심플":"dark minimal monochrome","파스텔":"soft pastel dreamy"};
  const color = colorMap[opts.imageColor] || "bright vibrant";
...
export default function App() {
  ...
  const [opts, setOpts] = useState({
    ...
    model:"Claude", geminiKey:"", imageStyle:"실사 사진형", imageColor:"밝고 경쾌한", imageText:"자막 포함",
  });
  ...
  const submitDirect = () => {
    ...
    pickTopic({ id:Date.now(), type:directType, title, source:"직접입력", from:"직접입력", ago:"방금", heat:100 });
    setDirectInput("");
  };
  const callAI = async (prompt) => {
    if (opts.model==="Claude") {
      ...
      const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${opts.geminiKey}`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:prompt}]}]})});
      const d = await r.json();
  ...
  const generate = async () => {
    if (!topic) return;
    if (opts.model==="Gemini"&&!opts.geminiKey) { alert("Gemini API Key를 입력하세요"); return; }
            ...
            <div>
              ...
              {topic && <>
                ...
                <OptCard title="AI 모델">
                  ...
                  {opts.model==="Gemini" && <input placeholder="Gemini API Key" value={opts.geminiKey} type="password" onChange={e=>setOpt("geminiKey",e.target.value)} style={{marginTop:8,width:"100%",background:C.s1,border:`1px solid ${C.border}`,borderRadius:6,padding:"6px 10px",color:C.text,fontSize:11,boxSizing:"border-box"}}/>}
                </OptCard>
